sft_unit_prompt = {
    ...
    输入报告：
    """,
    "肿瘤治疗": """你的任务是从输入报告中提取‘肿瘤治疗’相关信息，不要根据药品推理肿瘤具体治疗方式，报告中未提及的信息输出NA。若有多条肿瘤治疗，按照输出格式依次输出，不要漏提，输出格式::
    ...
    "治疗用药方案": """你的任务是从输入报告中提取‘治疗用药方案’相关信息，提取报告中所有的用药记录不要遗漏，报告中未提及的信息输出NA，停用的药不提取\
        分组依据：有治疗开始日期的以开始日期作为分组依据，没有开始日期的以每次用药作为依据。不要输出任何解释性语句,输出格式:
    [{
    治疗开始日期: '' //用药的开始日期，不要推断开始日期，若直接说明当日用药，提取该日期，否则输出NA。
    治疗结束日期: ''//用药的结束日期，不要推断结束日期，明确说明什么时候结束、完成用药再提取，否则输出NA。
    治疗用药名称: '' // 以列表形式列出,如果药品名后有括号，只输出括号外的药名，括号内的别名不输出。原文简写的治疗方案直接输出，如‘**治疗方案’、‘**方案’直接输出‘**’等。
    治疗用药是否为建议: '' //明确说明建议使用某些药输出‘是’，否则输出‘否’，没有用药信息输出NA。
    }]
    输入报告：
    """,
