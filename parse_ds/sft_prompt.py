sft_unit_prompt = {
    ...
    输出格式:[{
        "脑转移日期": string  // 
        "脑转部位": string  // 可选项为['大脑半球转移','丘脑转移','小脑转移','脑干转移','脑膜转移','未知','脑转移','无脑转']，该选项可多选，若有多个以列表形式输出。只有报告中明确出现“无脑转”或"未明显出现转 移"，"未知"等字眼，才输出相对应选项。
    }]//若有多条不同日期的影像学记录，继续以相同格式列出，并放到列表中.
    输入报告：""",
    "疾病": """"你的任务是提取报告中和‘疾病’相关的信息,按照要求输出结果。输出格式:[{
        "疾病首次确诊日期": string  // 
        "第一次病理确诊时间（穿刺、术后病理等）": string  // 
        "第一次切肺手术时间": string  // 
        "第一次影像确诊时间": string  // 
        "第一次治疗时间（药物、放疗等）": string  //        
        "首发症状时间": string  // 
        "疾病名称": string  // 可选项为'非小细胞肺癌','小细胞肺癌','胸膜间皮瘤','胸腺癌','腺癌','鳞屑样腺癌','腺泡样腺癌','乳头状腺癌','微乳头状腺癌','实性腺癌','浸润性黏液腺癌','混合性浸润性黏液性和非黏液性腺癌','胶体样腺癌','胎儿型腺癌','肠型腺癌','微浸润性腺癌','原位腺癌','黏液表皮样癌','腺样囊性癌','唾液腺型肿瘤','上皮肌上皮癌','鳞癌','角化型鳞状细胞癌','非角化型鳞状细胞癌','基底样鳞状细胞癌','原位鳞状细胞癌','大细胞癌','巨细胞癌','小细胞癌','复合型小细胞癌','腺鳞癌','侵 袭前病变','弥漫性特发性肺神经内分泌细胞增生','肉瘤样癌','多形性癌','梭形细胞癌','癌肉瘤','肺母细胞瘤','淋巴上皮瘤样癌','NUT癌','其他病理类型的非小细胞肺癌', '胸膜间皮瘤', '胸腺癌', '混合型非小细胞肺癌', '神经内分泌癌', '无'。
    }]""",
    "治疗用药方案": """你的任务是从输入报告中提取‘治疗用药方案’相关信息，报告中未提及的信息输出NA，出现连续的日期，首个日期为开始日期，最后一个日期为结束日期。按照以下要求直接输出json格式的结果，输出格式:
    [{
    治疗开始日期: '' //用药的开始日期，不要推断开始日期，若直接说明当日用药，提取该日期。
    治疗结束日期: ''//用药的结束日期，不要推断结束日期，明确说明什么时候结束再提取。
    治疗用药名称: '' // 以列表形式列出该治疗日期下的所有的药品名称，患者已停止服用的药品不输出。
    治疗用药是否为建议: '' //治疗用药名称是否为建议用药,明确说明建议使用某些药输出‘是’，否则输出‘否’
    } //如果有多条，按治疗日期分组继续以相同方式列出，不要遗漏。]
    输入报告：
    """,
    "基因检测": """你的任务是提取报告中和‘基因检测’相关的信息,按照输出格式的要求输出结果。特别注意：如果报告中的基因检测结果明确提到了突变+,但没有具体说明突变的类型，请选择“阳性”，如果提到了突变的类型，请直接选择突变的类型。
    :[{
        "基因检测日期": string  // 
        "EGFR": string  // 选项为["扩增", "18阳性", "19阳性", "20阳性", "21阳性", "19del", "19del/L858R", "L858R", "T790M", "20插入", "C797S", "G719X", "G719A", "G719D", "G719S", "G719C", "18del", "19ins", "S768I", "L861Q", "E709V", "其他罕见突变", "阳性"],19外显子突变”应等同于19del。
        "ALK": string  // 选项为["融合", "点突变", "重排", "插入", "扩增", "易位", "阳性"]
        "KRAS": string  // 选项为["扩增", "G13C", "G12A", "G12C", "G12X", "G213X", "Q61X", "G12D", "G12V", "G13D", "Q61L", "其他罕见突变", "阳性"]
        "BRAF": string  // 选项为['V600E','非V600','阳性']
        "MET": string  // 选项为['MET14跳突','MET扩增','c-MET过表达','c-MET扩增','其他罕见突变','MET14插入','MET14缺失','MET融合','阳性'],请注意'c-MET扩增'与'MET扩增'的区别,严格选项，切勿混淆。      
        "RET": string  // 选项为['融合','点突变','重排','插入','缺失','阳性'],“RET 17外显子突变”需要选择“点突变”。
        "ROS1": string  // 选项为['融合','点突变','重排','G3032R','S1986F','S1986Y','阳性']
        "NTRK": string  //["阳性", "融合", "点突变", "重排", "NTRK1", "NTRK2", "NTRK3"] “NTRK”是一个分层选项，例如出现“NTRK1 阳点突变/融合”，输出结果为“NTRK1 、点突变、融合”。
        "HER2(ERBB2)": string  // 选项为['20插入','非20插入','阳性']
        "FGFR": string  // 选项为['FGFR1','FGFR2','FGFR3','FGFR4','点突变','融合','重排','易位','阳性']，如病历中出现“FGFR1”，输出结果应该为'FGFR1 。
        "BRCA": string  // 选项为['BRCA1','BRCA2','阳性']
        "TP53": string  // 选项为['阳性']
        "KEAP1": string  // 选项为['阳性']
        "STK11": string  // 选项为['阳性']
        "HER4（ERBB4）": string  // 选项为['阳性']
        "RB1": string  // 选项为['阳性']
        "HER3（ERBB3）": string  // 选项为['阳性']
