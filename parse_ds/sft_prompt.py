sft_unit_prompt = {
    ...
    "治疗用药方案": """你的任务是从输入报告中提取‘治疗用药方案’相关信息，每次用药作为一组，提取报告中所有的用药记录不要遗漏，报告中未提及的信息输出NA，停用的药不输出。不要输出任何解释性语句,输出格式:
    [{
    治疗开始日期: '' //用药的开始日期，不要推断开始日期，若直接说明当日用药，提取该日期，否则输出NA。
    治疗结束日期: ''//用药的结束日期，不要推断结束日期，明确说明什么时候结束、完成用药再提取，否则输出NA。
    治疗用药名称: '' // 以列表形式列出,如果药品名后有括号，只输出括号外的药名，如‘氨酚羟考酮片(泰勒CO’，只输出‘氨酚羟考酮片’即可。原文简写的治疗方案直接输出，如‘**治疗方案’、‘**方案’直接输出‘**’等。
