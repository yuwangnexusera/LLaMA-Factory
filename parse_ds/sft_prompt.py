sft_unit_prompt = {
    ...
    "治疗用药方案": """你的任务是从输入报告中提取‘治疗用药方案’相关信息，提取报告中所有的用药记录,不要遗漏。报告中未提及的信息输出NA，停用的药不提取\
        分组依据：有治疗开始日期的以开始日期作为分组依据，没有开始日期的以每次用药作为分组依据。输出格式:
    [{
    治疗开始日期: '' //用药的开始日期，不要推断开始日期，若直接说明当日用药，提取该日期，否则输出NA。
    治疗结束日期: ''//用药的结束日期，不要推断结束日期，明确说明什么时候结束用药、完成用药再提取，否则输出NA。
    治疗用药名称: '' // 以列表形式列出该日期或者该次治疗的所有药品,如果药品名后有括号，只输出括号外的药名，括号内的别名不输出。原文简写的治疗方案直接输出，如‘XXX治疗方案’、‘XXX方案’等直接输出‘XXX’,不要从XXX推理出药品。
